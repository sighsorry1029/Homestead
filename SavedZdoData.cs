...
internal sealed class SavedZdoData
{
    private const string Prefix = "hs-zdo-v1:";
    ...
    public SavedZdoData(ZDO zdo)
    {
        Load(zdo);
    }

    public SavedZdoData(string payload)
    {
        Load(payload);
    }
    ...
    private void Load(string payload)
    {
        ...
        {
            throw new InvalidDataException("Unsupported Homestead ZDO data format. Re-save this blueprint with the current Homestead version.");
        }
        ...
        if (version != Version)
        {
            throw new InvalidDataException($"Unsupported Homestead ZDO data version {version}.");
        }
...
{
    public static ZDO? Init(GameObject prefab, Vector3 position, Quaternion rotation, Vector3? scale, SavedZdoData data, Dictionary<string, string> _)
    {
