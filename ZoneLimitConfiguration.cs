...
internal static class ZoneLimitConfiguration
{
    ...
    public static void ReloadFromDisk()
    {
        ...
        if (!HomesteadPlugin.ConfigSync.IsSourceOfTruth)
        {
        ...
        string yaml = File.ReadAllText(HomesteadPlugin.ZoneRuleFileFullPath);
    ...
    public static bool IsArchiveProtected(long playerId, out string reason)
    {
        ...
        if (TryNormalizeSteamPlatformId(record.PlatformId, out string steamId) &&
    ...
    {
        if (File.Exists(HomesteadPlugin.ZoneRuleFileFullPath))
        {
            return;
        }

        Directory.CreateDirectory(Path.GetDirectoryName(HomesteadPlugin.ZoneRuleFileFullPath)!);
        File.WriteAllText(HomesteadPlugin.ZoneRuleFileFullPath, BuildDefaultYaml());
    }
    ...
    private static void OnSyncedYamlChanged()
    {
        ...
        ApplyYaml(_syncedYaml.Value, HomesteadPlugin.ConfigSync.IsSourceOfTruth ? "local file" : "server sync");
    }
    ...
    private static void ApplyConfiguration(List<ZoneLimitRule> parsedRules, ZoneArchiveProtection parsedProtection)
    {
        ...
        {
            string normalized = NormalizeSteamId(steamId);
    ...
    private static string NormalizeSteamId(string value)
    {
        ...
        return raw.Length == 17 && raw.All(char.IsDigit) ? raw : "";
    }
    ...
    public static bool TryCreateFromFile(ZoneArchiveProtectionFile? file, out ZoneArchiveProtection protection, out string error)
    {
        ...
        for (int i = 0; i < (file?.SteamIds?.Count ?? 0); i++)
        {
            ...
            if (string.IsNullOrWhiteSpace(steamId) ||
                steamId.Trim().Length != 17 ||
                !steamId.Trim().All(char.IsDigit))
            {
