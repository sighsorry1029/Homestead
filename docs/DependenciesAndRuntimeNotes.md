...
2. Admin checks saved archive tags with `zs_archive_list`.
3. Admin restores a whole archived manifest shape with RCON/server console:

```text
zs_loadarchive auto_p123456789_s76561198000000000_c001_x-10_z5 to (20,-3)
```

The target zone is treated as the lower-left/min X/min Z anchor of the saved manifest. Non-rectangular clusters keep their original shape because `zs_loadarchive` loads the manifest's explicit zone list rather than a rectangular range.

Zone bundle terrain handling is SupportFill-only. `zs_savezone`, `zs_loadzone`, and `zs_loadarchive` no longer accept a `--terrain` mode override.
