...
internal static class AutoArchiveStore
{
    ...
    public static string FilePath => Path.Combine(HomesteadPlugin.DataStorageFullPath, HomesteadPlugin.ActivityFileName);
    ...
    public static void RecordPlayerSeen(string platformId, long playerId, string name, DateTime utcNow)
    {
        ...
        lock (Sync)
        {
            ...
            if (record.FirstSeenUtc == DateTime.MinValue)
            {
                record.FirstSeenUtc = utcNow;
            }
    ...
    public static void RecordManualPlayerSeen(long playerId, DateTime utcNow)
    {
        lock (Sync)
        {
            PlayerActivityRecord record = PlayersById.TryGetValue(playerId, out PlayerActivityRecord indexed)
                ? indexed
                : new PlayerActivityRecord
                {
                    PlatformId = ManualPlatformId(playerId),
                    FirstSeenUtc = utcNow,
                    PlayerIds = [playerId]
                };
            if (!State.Players.Contains(record))
            {
                State.Players.Add(record);
            }
            ...
            if (IsUnknown(record))
            {
                record.PlatformId = ManualPlatformId(playerId);
            }
            ...
            record.LastSeenUtc = utcNow;
            AddDistinct(record.Names, "manual");
    ...
    public static bool TryGetPlayerIdsBySteamId(string steamId, out List<long> playerIds, out string normalizedSteamId)
    {
        ...
        lock (Sync)
        {
            ...
            {
                if (!TryNormalizeSteamPlatformId(record.PlatformId, out string recordSteamId) ||
        ...
        string observedName = "")
    {
        ...
        if (result.Ignored)
        {
            result.Reason = $"player {playerId} is ignored";
            return result;
        }
        ...
        DateTime reference = unknown ? record.FirstSeenUtc : record.LastSeenUtc;
        int requiredDays = unknown ? unknownGraceDays : inactiveDays;
        ...
        if (age.TotalDays < requiredDays)
        {
            result.Reason = unknown
                ? $"unknown player {playerId} first seen {age.TotalDays:F1}/{requiredDays} days ago"
                : $"player {playerId} last seen {age.TotalDays:F1}/{requiredDays} days ago";
            return result;
        }

        result.Eligible = true;
        result.Reason = unknown
            ? $"unknown player {playerId} grace expired"
    ...
    private static void MergePlayerIdRecords(PlayerActivityRecord target, long playerId)
    {
                     ...
                     .ToList())
        {
            ...
            if (target.FirstSeenUtc == DateTime.MinValue || duplicate.FirstSeenUtc < target.FirstSeenUtc)
            {
                target.FirstSeenUtc = duplicate.FirstSeenUtc;
            }
    ...
    private static string ManualPlatformId(long playerId)
    {
        return ManualPlatformPrefix + playerId;
    }
    ...
    private static bool IsManual(PlayerActivityRecord record)
    {
        return record.PlatformId.StartsWith(ManualPlatformPrefix, StringComparison.Ordinal);
    }

    private static bool TryNormalizeSteamPlatformId(string platformId, out string steamId)
    {
        steamId = "";
        if (string.IsNullOrWhiteSpace(platformId) ||
            !platformId.StartsWith("steam:", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }

        steamId = NormalizeSteamId(platformId.Substring("steam:".Length));
