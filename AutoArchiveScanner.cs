...
internal static class AutoArchiveScanner
{
    ...
    public static IEnumerator Run(AutoArchiveScanOptions options, Action<ArchiveRunRecord> onComplete)
    {
        ...
        Dictionary<Vector2i, AutoArchiveZoneInfo> candidates = BuildCandidateZones(zoneInfos, utcNow, options.TargetPlayerIds, run);
        ...
        foreach (List<Vector2i> cluster in clusters.OrderBy(cluster => cluster.Min(zone => zone.x)).ThenBy(cluster => cluster.Min(zone => zone.y)))
        {
            ...
            bool smallCluster = record.PieceCount < AutoArchiveConfig.MinimumPiecesPerCluster;

            if (smallCluster && AutoArchiveConfig.SmallClusterAction == AutoArchiveSmallClusterAction.Skip)
            {
                record.Status = "skipped";
                record.Reason = $"piece count {record.PieceCount} is below minimum {AutoArchiveConfig.MinimumPiecesPerCluster}";
            ...
            if (!AllCreatorsEligible(record.Creators, utcNow, out string creatorReason))
            {
                record.Status = "skipped";
            ...
            if (smallCluster && AutoArchiveConfig.SmallClusterAction == AutoArchiveSmallClusterAction.ResetWithoutSave)
            {
            ...
            if (options.DryRun)
            {
                ...
                record.Reason = "candidate only; dry run is enabled";
                run.Clusters.Add(record);
                processedZones += cluster.Count;
                continue;
            }
            ...
            if (options.ResetAfterSave)
            {
                if (AutoArchiveConfig.RequireLoadedTerrainForReset && saveResult.TerrainCaptured < cluster.Count)
                {
                    record.Status = "saved-reset-skipped";
                    record.Reason = $"saved, but reset skipped because usable terrain contacts were captured for {saveResult.TerrainCaptured}/{cluster.Count} zones";
                }
                else
                {
    ...
    private static bool TryReadPlayerStructure(ZDO zdo, DateTime utcNow, out Vector2i zone, out long creator)
    {
        ...
        if (prefab == null ||
            ...
            !ZoneBlueprintCommands.HasBuildRecipe(prefab))
        {
            return false;
        }
    ...
    private static Dictionary<Vector2i, AutoArchiveZoneInfo> BuildCandidateZones(
        Dictionary<Vector2i, AutoArchiveZoneInfo> zoneInfos,
        DateTime utcNow,
        IReadOnlyCollection<long> targetPlayerIds,
        ArchiveRunRecord run)
    {
        Dictionary<Vector2i, AutoArchiveZoneInfo> candidates = [];
        foreach (AutoArchiveZoneInfo info in zoneInfos.Values)
        {
            if (targetPlayerIds.Count > 0 && !info.Creators.Any(targetPlayerIds.Contains))
            {
                continue;
            }
    ...
    private static ArchiveClusterRecord BuildClusterRecord(List<Vector2i> cluster, Dictionary<Vector2i, AutoArchiveZoneInfo> candidates)
    {
        ArchiveClusterRecord record = new()
        {
            Zones = cluster
                .OrderBy(zone => zone.y)
                .ThenBy(zone => zone.x)
                .Select(zone => new ZoneBundleZone { X = zone.x, Z = zone.y })
                .ToList()
        };
    ...
    private static string SanitizeTagToken(string value)
    {
        ...
        {
            sanitized = sanitized.Substring(0, 32).Trim('_');
        }
