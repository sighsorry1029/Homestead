...
{
    private static readonly int ZoneBlueprintPlacedHash = StringExtensionMethods.GetStableHashCode(HomesteadPlugin.ModGUID + ".blueprint_piece");
    ...
    public static bool CanAddWearNTears(IEnumerable<Vector3> positions, out string reason)
    {
        ...
        foreach (KeyValuePair<Vector2i, int> item in additions)
        {
            ...
            {
                reason = HomesteadLocalization.Format("hs_zone_limit_exceeded", item.Key.x, item.Key.y, projected, rule.Limit);
                return false;
            }
        }
    ...
    private static bool TryBuildStructureState(GameObject gameObject, out StructureState state)
    {
        ...
        bool forceCount = IsKnownBlueprintPiece(zdo);
    ...
    private static bool IsKnownBlueprintPiece(ZDO zdo)
    {
        return zdo.GetBool(ZoneBlueprintPlacedHash, false);
    }
    ...
    {
        string message = HomesteadLocalization.Format("hs_zone_limit_reached", zoneCount, limit);
